...
from esl.forms import (
    AddAdvancedSign,
    AddAdvancedVideoLecture,
    AddBeginnerSign,
    AddBeginnerVideoLecture,
    AddIntermediateSign,
    AddIntermediateVideoLecture,
    SignupInstructorForm,
)
...
def instructor_feedback(request):
    ...
    status = True
    context = {"status": status, "feedback_data": feedback_data}
...
def instructor_feedback_save(request):
    if request.method != "POST":
        ...
        try:
            add_feedback = FeedBackInstructor(
                Instructor_id=instructor_obj, feedback=feedback, feedback_reply=""
            )
            add_feedback.save()
